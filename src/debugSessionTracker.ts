...
export class DebugSessionTracker {
    ...
    private mode: 'playback' | 'recording' = 'playback';
    private recordedFunctions: Map<string, string> = new Map(); // name -> code
    private callGraph: { caller: string, callee: string }[] = [];
    private lastFunction: string | null = null;
    ...
    private autoBreakpoints: vscode.Breakpoint[] = [];
    ...
    private async clearAutoBreakpoints() {
    ...
    private triggerDebouncedStepInto(stackItem?: unknown) {
    ...
    private async handleStepInto(passedStackItem?: unknown) {
        ...
        try {
            ...
            if (passedStackItem && typeof passedStackItem === 'object' && 'source' in passedStackItem) {
                ...
                const frame = passedStackItem as any;
                trace = [{
                    name: frame.name,
            ...
            // 2. Fallback to current active stack item
                ...
                const frame = vscode.debug.activeStackItem as any;
                trace = [{
                    name: frame.name,
                    source: { path: frame.source.path },
                    line: frame.line,
            ...
            if (!topFrame.source || !topFrame.source.path) return;
    ...
    private async processRecordedSession(): Promise<number> {
        ...
        for (const [name, code] of this.recordedFunctions) {
            try {
                const explanation = await this.aiService.explainFunction(
                    code,
                    name,
                    panorama,
                    'Dynamic Trace Context' // We could enhance this with specific call paths
                );
    ...
    private generateDynamicPanorama(): string {
    ...
    private isInWorkspace(filePath: string): boolean {
        try {
            ...
            const checkPath = (pathStr: string): boolean => {
                let uri: vscode.Uri;
                if (pathStr.includes('://')) {
            ...
            // Try 1: Original path
            if (checkPath(filePath)) return true;
            ...
            if (decodedPath !== filePath) {
                ...
                if (checkPath(decodedPath)) return true;
            }
    ...
    private async getSourceCode(frame: any): Promise<string> {
    ...
    private extractFunctionAtLine(content: string, lineIndex: number): string {
        const lines = content.split('\n');
