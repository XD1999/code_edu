...
export class DebugSessionTracker {
    ...
    private async getStackTrace(): Promise<any[]> {
        ...
        try {
            ...
            const response = await this.activeSession.customRequest('stackTrace', {
                threadId: 1, // Main thread, in most cases
                startFrame: 0,
    ...
    private async getWorkspaceFileStructure(): Promise<any> {
    ...
    private async extractFunctionsFromTrace(trace: any[]): Promise<{ name: string, code: string }[]> {
                ...
                if (!sourcePath.includes('node:internal') &&
                    ...
                    try {
                        ...
                        const sourceResponse = await this.activeSession?.customRequest('source', {
                            source: frame.source,
                            startLine: Math.max(0, frame.line - 5), // Get 5 lines before
    ...
    private async explainFunctions(functions: { name: string, code: string }[], trace: any[]): Promise<any[]> {
