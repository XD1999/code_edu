...
class DebugSessionTracker {
    ...
    async captureStackFunctions(session) {
        ...
        try {
            ...
            console.log('[THREADS] Thread summary:', threads.map((t) => `[ID:${t.id}, Name:"${t.name}"]`).join(', '));
            // Detect new threads for better worker thread awareness
            const currentThreadIds = new Set(threads.map((t) => t.id));
            const newThreadIds = threads.filter((t) => !sessionPreviousThreadIds.has(t.id)).map((t) => t.id);
            ...
            const threadPromises = threadsToProcess.map(async (th) => {
            ...
            const threadStats = threadResults.filter(result => result !== null);
