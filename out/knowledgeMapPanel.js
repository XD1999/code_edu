...
class KnowledgeMapProvider {
    ...
    _generateContextArch() {
        ...
        this._archPromise = this._onContextArch(text)
            ...
            if (hasEdges || hasBoxes) {
                ...
                vscode.window.showWarningMessage('No logic framework could be generated for this context. Term explanations will appear without an arch framework.');
            }
        })
            .catch(e => {
            console.error('Context arch generation error:', e);
            vscode.window.showErrorMessage('Failed to generate the logic framework (arch): ' + (e?.message || e));
        });
    }
     ...
     */
    addTerm(term, explanation, type = 'desc-encapsulation', focusSnapshot) {
    ...
    async processInputTerm(term, type = 'desc-encapsulation', autoVisualize = false) {
            ...
            if (termToDelete) {
                ...
                for (const branch of termToDelete.branches || []) {
                    if (branch.childContext) {
                        (0, util_1.collectTermNames)(branch.childContext, subtreeNames);
                    }
                }
            }
     ...
     */
    _pruneTermsFromGraph(termNames) {
        if (!this._knowledgeGraph || this._knowledgeGraph.edges.length === 0)
            return;
        const norm = (s) => String(s || '').trim().toLowerCase().replace(/\s+/g, ' ');
        const toRemove = new Set(termNames.map(norm).filter(Boolean));
        if (toRemove.size === 0)
            return;
        const edges = this._knowledgeGraph.edges.filter(e => !toRemove.has(norm(e.source)) && !toRemove.has(norm(e.target)));
    ...
    _findTermById(context, id) {
        return (0, util_1.findTermById)(context, id);
    }
    _collectVisualizationFiles(term, files) {
        (0, util_1.collectVisualizationFiles)(term, files);
    }
    _recursiveDeleteTerm(context, termId) {
        return (0, util_1.recursiveDeleteTerm)(context, termId);
    }
    getKnowledgeGraph() {
     ...
     */
    async elaborateGraphForTerm(term, contextText, aiService) {
        try {
            ...
            if (prev.length > 0) {
                const key = (e) => `${e.source}${e.target}${e.relation}`;
    ...
    _collectTermNames(context, names) {
        (0, util_1.collectTermNames)(context, names);
    }
    _buildMermaidGraph(graph) {
